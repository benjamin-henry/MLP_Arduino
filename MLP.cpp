...
unsigned int mlp_save_config(MLP mlp, unsigned int address) {
    // first slot is for input_shape, others are for units and activation for each layer
    EEPROM.writeUInt(address, mlp->_input_shape);
    address += sizeof(unsigned int);
    
    for(unsigned int i = 0; i < mlp->_n_layers; i++) {
        EEPROM.writeUInt(address, mlp->_layers[i]->_units);
        address += sizeof(unsigned int);

        EEPROM.writeUInt(address, mlp->_layers[i]->_activation);
        address += sizeof(unsigned int);
    }
    return address;
}
...
unsigned int mlp_save_weights(MLP mlp, unsigned int address) {
    unsigned int i,j,k;
    for(i = 0; i < mlp->_n_layers; i++) {
        // save weights
        for(j = 0; j < mlp->_layers[i]->_input_shape; j++) {
            for(k = 0; k < mlp->_layers[i]->_units; k++) {
                EEPROM.writeFloat(address, mlp->_layers[i]->_w[j][k]);
        ...
        // save biases
        for(k = 0; k < mlp->_layers[i]->_units; k++) {
            EEPROM.writeFloat(address, mlp->_layers[i]->_b[k]);
            address += sizeof(float);
        }
    }
    return address;
...
unsigned int mlp_load_weights(MLP mlp, unsigned int address) {
    ...
    for(i = 0; i < mlp->_n_layers; i++) {
        ...
        for(j = 0; j < mlp->_layers[i]->_input_shape; j++) {
            for(k = 0; k < mlp->_layers[i]->_units; k++) {
                mlp->_layers[i]->_w[j][k] = EEPROM.readFloat(address);
                address += sizeof(float);
            }
        }
        ...
        // load biases
        for(k = 0; k < mlp->_layers[i]->_units; k++) {
            mlp->_layers[i]->_b[k] = EEPROM.readFloat(address);
            address += sizeof(float);
        }
    }
    return address;
